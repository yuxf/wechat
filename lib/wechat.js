...
var checkSignature = function (query, token) {
  ...
  console.log(arr);
  console.log("check ", shasum.digest('hex'), " and expects ", signature)
    ...
    load(req, function (err, buf) {
      ...
      xml2js.parseString(xml, {trim: true}, function (err, result) {
        ...
        if (signature !== cryptor.getSignature(timestamp, nonce, encryptMessage)) {
          console.log("signature mismatch, expect is " + cryptor.getSignature(timestamp, nonce, encryptMessage) + ", but from request we get " + signature)
          res.writeHead(401);
          res.end('Invalid signature');
          return;
        }
...
Handler.prototype.middlewarify = function () {
  ...
  return function (req, res, next) {
	...
	console.log("checkSignature error", req.query);
        res.writeHead(401);
        res.end('Invalid signature');
        return;
      }
